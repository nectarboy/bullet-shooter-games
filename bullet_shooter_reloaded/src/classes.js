...
class AnimManager {
    ...
    update() {
        ...
        if (anim.oneFrame || this.tick++ < anim.speed)
            return;
        this.tick = 0;
...
class Player extends GravityEntity {
    ...
    updateAnimation() {
        ...
        if (this.grounded) {
            // running animation
            if (this.moving) {
                this.animManager.switchAnim(1);
            }
            else {
                // cute lil crouch animation :3
                if (this.crouching)
                    this.animManager.switchAnim(2);
                // idle animation :3
                else
                    this.animManager.switchAnim(0);
            }
        }
        else {
