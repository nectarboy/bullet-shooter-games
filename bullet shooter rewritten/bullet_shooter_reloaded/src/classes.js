...
class Sprite {
    ...
    centerOnto(x, y, w, h) {
        this.x = x - (this.scale * this.w)/2;
        this.y = y - (this.scale * this.h)/2;
    }
...
class Entity {
    ...
    isInside(x, y, w, h) {
        return (
            this.x > x - this.w && this.x < x + w &&
            this.y > y - this.h && this.y < y + h
        );
    }
...
class GravityEntity extends Entity {
    ...
    updateVelocity(xdir) {
        ...
        // update y
        this.vy += this.ay;
        if (this.vy > this.sy) this.vy = this.sy;
    }
...
class Player extends GravityEntity {
    constructor(x, y) {
        super(x, y, 12, 14, 2, 3, 0.2, 0.6, 0.3, 0.7);
        ...
        this.sprite = new Sprite(assets.player_spritesheet, 3, 3, 16, 16, 0, 1);
    }
    ...
    // movement 
    checkMoving() {
        var xdir = joypad.right - joypad.left;
        this.moving = xdir !== 0;
