...
var world = {
	...
	background: '#2e2e2e',
	paused: false,
	godMode: false // turn this to true for the ability to place blocks and be invincible
};
			...
			conditions.push({
				...
				check: function() {
					...
					if(this.count >= this.max) {
						//if(!ent.alive) { // Is this condition causing the bug? I'll never know...too bad!
...
function damage(hitter, victim) {
	if(hitter.alive && !victim.invincible && !world.godMode) {
		...
		if(damage < 1)
			...
			conditions.push({
				num: 0,
				max: victim.type != type.entity[0] ? 13 : 35,
				check: function() {
					this.num++
...
function spawnEntity() { // At first i thought about hardcoding all the properties in... imaaagine lmaooo
...
class biped {
	...
	if(this.alive) {
		...
		if(wasOnFloor && !this.onG && this.spY > 0 && this.type == type.entity[0]) { // coyote jumping is back!
			this.canJump = true;
			var ent = this;
			conditions.push({
				coyote: 0,
	...
	hostileBehavior(ent) {
		...
		if(this.onG) {
			if(this.counter >= this.coolDown)
				this.jumpUp(this.jump, (ent.x - this.x)*0.2);
		} else
			this.counter = 0;
...
spawnEntity();
/* I know that this will declare all the spawnfuncts globally; but for testing purposes this is left on,
this will be declared in functions, left to be garb collected, later on anyways. */
